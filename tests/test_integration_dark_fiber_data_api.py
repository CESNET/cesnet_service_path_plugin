...
def test_segment_includes_technical_data(base_segment):
    ...
    assert data["type_specific_technicals"]["fiber_mode"] == "single_mode"
    assert float(data["type_specific_technicals"]["total_loss"]) == 8.5
...
def test_update_dark_fiber_data(base_segment):
    ...
    assert float(seg_data["type_specific_technicals"]["total_loss"]) == 7.2
